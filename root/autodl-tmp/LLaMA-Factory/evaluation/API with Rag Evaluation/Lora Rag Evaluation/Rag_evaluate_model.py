...
def main():
    ...
    test_path = "/root/autodl-tmp/LLaMA-Factory-Fine-Tune/data/test.jsonl"
    api_port = 8000
    ...
    try:
        ...
        if results:
            print(f"\n=== Lora Model RAG evaluation results ===")
            print(f"Total number of questions: {results['total_count']}")
            print(f"Correct predictions: {results['correct_count']}")
            print(f"Accuracy: {results['accuracy']:.4f} ({results['accuracy']*100:.2f}%)")
            print(f"API Error Count: {results['api_error_count']}")
            print("======================")
        else:
            print("评估失败，请检查日志文件")
